...
/**
 * Custom highlighter from the timeline graph.
 */
export class TimelineHighlightManager extends HighlightIndicatorManager<INode> {
  private readonly nodeHighlightGroup: any
  ...
  getInstaller(item) {
    if (INode.isInstance(item)) {
      return new NodeStyleDecorationInstaller({
        margins: new Insets(1),
        zoomPolicy: StyleDecorationZoomPolicy.MIXED,
        nodeStyle: new ShapeNodeStyle({
          fill: Fill.ORANGE_RED,
          stroke: new Stroke({
            thickness: 2,
            fill: Fill.ORANGE
          })
